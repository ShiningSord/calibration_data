...
class CustomAttn(nn.Module):
    def __init__(self, layer_id, args: Params):
        ...
        self.k_proj = nn.Linear(args.dim, args.n_heads * self.head_dim, bias=False)
        self.v_proj = nn.Linear(args.dim, args.n_heads * self.head_dim, bias=False)
    ...
    def forward(self, x: torch.Tensor, is_causal=True, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        '''
        q_w, k_w, v_w = self.in_proj.weight.chunk(3,dim=0)
        queries = x@q_w.T
        keys = x@k_w.T
        vals = x@v_w.T
        '''
...
class Block(nn.Module):
    ...
    def reset_parameters(self):
        if self._ffn_type == "swiglu" or self._ffn_type == "swiglu_torch":
            ...
            torch.nn.init.trunc_normal_(self.feed_forward.w2.weight, std=std, a=-3 * std, b=3 * std)
            # scale init by depth as in https://arxiv.org/abs/1908.11365 -- worked slightly better.
            std = 1.0 / math.sqrt(self.hidden_dim)
            std = std / math.sqrt(2 * (self.layer_id + 1))
    ...
    def forward(self, input_ids=None, inputs_embeds=None, past_key_values=None, use_cache=False, attention_mask=None):
        ...
        for i, layer in enumerate(self.layers):
            if self.grad_checkpointing:
                ...
                x, past_key_values[i] = layer(x, attention_mask=attention_mask)#, past_key_values[i], use_cache=use_cache)
