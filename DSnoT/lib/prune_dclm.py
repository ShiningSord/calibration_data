...
def prepare_calibration_input(args, model, dataloader, device):
    ...
    inps = torch.zeros(
        (args.nsamples, model.seqlen, model.config.dim),
        dtype=dtype,
        device=device,
    )
...
def prune_DSnoT(
    ...
    dataloader, _ = get_loaders(
        "slimpajama", nsamples=args.nsamples, seed=args.seed, seqlen=2048, tokenizer=tokenizer
    )
    '''
    dataloader, _ = get_loaders(
        "dclm", nsamples=args.nsamples, seed=args.seed, seqlen=2048, tokenizer=tokenizer
    )
    '''
