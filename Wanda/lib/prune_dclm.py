...
def prepare_calibration_input(args, model, dataloader, device):
    ...
    inps = torch.zeros((args.nsamples, model.seqlen, model.config.dim), dtype=dtype, device=device) # ori: 128
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    if 'sampled' in args.data:
        data = torch.load(args.sampled_path)
...
def prune_sparsegpt(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    ...
    dtype = next(iter(model.parameters())).dtype
    inps = torch.zeros(
        (args.nsamples, model.seqlen, model.config.dim), dtype=dtype, device=dev
    )
    cache = {'i': 0, 'attention_mask': None, "position_ids": None}
