...
sample_mode='selfgen'
nsamples=128
output_file="dclm_7b_${sample_mode}_${data}_ratio${sparsity_ratio}_type${sparsity_type}_nsamples${nsamples}_results.txt"
...
for seed in {0..4}; do
    ...
    CUDA_VISIBLE_DEVICES=0 python main_dclm_sample.py \
        --model $model \
        --prune_method wanda \
        --seed $seed_value \
        --data $data \
