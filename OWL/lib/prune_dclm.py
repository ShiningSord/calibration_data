...
def prepare_calibration_input(args, model, dataloader, device):
    ...
    dtype = next(iter(model.parameters())).dtype
    inps = torch.zeros((args.nsamples, model.seqlen, model.config.dim), dtype=dtype, device=device)
    inps.requires_grad = False
...
def prune_wanda_outlier_structure_special(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    dataloader, _ = get_loaders(args.data,nsamples=args.nsamples, seed=args.seed, seqlen=2048, tokenizer=tokenizer)
